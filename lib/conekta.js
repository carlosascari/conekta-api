...
Conekta.prototype.Customer = {
	...
	{
		this.apiRequest('delete', {url: format("%s/%s", CUSTOMER_RES, customer_id), headers: HEADERS}, callback)
	}.bind(instance),
}
...
Conekta.prototype.Card = {
	...
	{
		this.apiRequest('delete', {url: format("%s/%s/cards/%s", CUSTOMER_RES, customer_id, card_id), headers: HEADERS}, callback)
	}.bind(instance),
}
...
Conekta.prototype.Subscription = {
	create: function(customer_id, plan_id, card_token, callback)
	{
		if (arguments.length === 3) callback = card_token, card_token = undefined
...
Conekta.prototype.Plan = {
	...
	{
		this.apiRequest('delete', {url: format("%s/%s", PLAN_RES, plan_id) , headers: HEADERS}, callback)
	}.bind(instance),
}
